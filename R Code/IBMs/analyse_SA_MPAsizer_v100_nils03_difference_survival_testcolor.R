...
for (scenario in 1:dim(para.scenarios)[1]){ # loop over scenarios
  ...
  for(rd in 1:length(returndat)){
    ...
    for (plotnum in 1:length(fmortcols)){
      ...
      # save results in csv
      lfmorts.print <- round(lfmorts[,,plotnum,scenario]*100)
      colnames(lfmorts.print) <- MPAsizes
      rownames(lfmorts.print) <- species.names.ordered
      write.csv(lfmorts.print,paste0('Survival_probabilities_F',fmortplot[plotnum]*100,'_',age,'_MeanMaxDist',mean.max.dist,'.csv')) 
    ...
    if(scenario==2){
      ...
      colnames(lfmorts.diff21) <- MPAsizes
      rownames(lfmorts.diff21) <- species.names.ordered
    }
    if(scenario==3){
      lfmorts.diff32 <- lfmorts[,,,3]-lfmorts[,,,2] # compare to mean scenario
      colnames(lfmorts.diff32) <- MPAsizes
      rownames(lfmorts.diff32) <- species.names.ordered
    }
    if(scenario==3){
      lfmorts.diff31 <- lfmorts[,,,3]-lfmorts[,,,1] # compare to mean scenario
      colnames(lfmorts.diff31) <- MPAsizes
      rownames(lfmorts.diff31) <- species.names.ordered
    }
    
} # scenario loop
    
write.csv(round(lfmorts.diff21*100),'difference_survival_probabilities_longevity_movements.csv') 
write.csv(round(lfmorts.diff31*100),'difference_survival_probabilities_longevity_maturity.csv') 
write.csv(round(lfmorts.diff32*100),'difference_survival_probabilities_movements_maturity.csv') 
...
# ucols <- c("PuBu","Black","Oranges"); 
# ltys <- rep(1,sum(ngroups)); 
# cols <- NA
# for(g in 1:length(ngroups)){
#   #ltys <- c(na.omit(ltys),1:ngroups[g])
#   if(ngroups[g]>1) {
#     cols <- c(na.omit(cols),brewer.pal(ngroups[g]+2,ucols[g])[3:(ngroups[g]+2)])
#     } else {cols <- c(na.omit(cols),ucols[g])} # specify colours
# }


ultys <- c(1,2,3) #rep(1,sum(ngroups)); 
cols <- brewer.pal(sum(ngroups),"Paired")
ltys <- NA
for(g in 1:length(ngroups)){
  ltys <- c(na.omit(ltys),rep(ultys[g],ngroups[g]))
}
      ...
      #xlims <- c(10,100)
        ...
        #for(xl in 1:length(xlims)){
          ...
          #xlim <- xlims[xl]
            ...
            par(mar=c(4.1, 4.1, 1.1, 2.1)) # this is usually the default
            ...
            plot(MPAsizes/1000,lfmorts[1,,plotnum,scenario] * 100,
                 ...
                 xlab = NA, ylab = s.ylabel,cex=cex.labels,las=1,bty='l') # xlab = xlabel.size, 
            ...
            if (scenario>1){
              ...
              ylim.min <- 0 #floor(min(slfmorts.diff[,1:xlim.loc,plotnum],na.rm=TRUE)*10)*10
              ...
              plot(MPAsizes/1000,slfmorts.diff[1,,plotnum] * 100,
                   ...
                   xlab = NA,ylab = s.ylabel2,las=1,bty='l') # xlab = xlabel.size, 
              #main = paste0('Age at ',age,': ',fmortplot[plotnum] * 100, '% annual fishing mortality'))
