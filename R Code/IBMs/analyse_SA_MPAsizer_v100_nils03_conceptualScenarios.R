...
MPAsizes <- c(seq(500,10000,500),seq(20000,50000,10000)) #c(0,seq(500,2000,500),seq(3000,5000,1000),seq(10000,30000,10000),50000,100000) # specify MPA sizes to be analysed (in m)
...
fmortplot <- c(1,1) #c(0.05,0.2,0.5,1) # lifetime mortality plots
...
dir.create(scenario.results.folder)
...
# check results
#xlim<-10000
#id<-1;plot(MPAsizes,returndat[[id]]$statstable$mean_nfull/returndat[[id]]$statstable$mean_npart,type='l',xlim=c(0,xlim))
#id<-2;lines(MPAsizes,returndat[[id]]$statstable$mean_nfull/returndat[[id]]$statstable$mean_npart,col='red')
#id<-3;lines(MPAsizes,returndat[[id]]$statstable$mean_nfull/returndat[[id]]$statstable$mean_npart,col='blue')
...
lfmorts <- array(dim=c(nspecies,length(MPAsizes),length(fmortplot)))
  ...
  species.names.ordered <- species.names # get names of species in order
  ...
  for(rd in 1:length(returndat)){
    ...
    for (plotnum in 1:length(fmortcols)){
      ...
      lfmorts[1,,plotnum] <- returndat[[1]]$statstable$mean_nfull/returndat[[1]]$statstable$mean_npart #[,fmortcols[plotnum]]
      #lfmorts[1,,plotnum] <- returndat[[1]]$statstable$mean_prot
      #lfmorts[1,,plotnum] <- returndat[[1]]$statstable$median_prot
      #lfmorts[1,,plotnum] <- returndat[[1]]$statstable$mean_nfull/returndat[[1]]$statstable$mean_n #[,fmortcols[plotnum]]
      
      for (s in 1:nspecies) {
        #lfmorts[s,,plotnum] <- 1-returndat[[s]]$mean_Fmortality[,fmortcols[plotnum]]
        lfmorts[s,,plotnum] <- returndat[[s]]$statstable$mean_nfull/returndat[[s]]$statstable$mean_npart #[,fmortcols[plotnum]]
        #lfmorts[s,,plotnum] <- returndat[[s]]$statstable$mean_prot
        #lfmorts[s,,plotnum] <- returndat[[s]]$statstable$median_prot
        #lfmorts[s,,plotnum] <- returndat[[s]]$statstable$mean_nfull/returndat[[s]]$statstable$mean_n #[,fmortcols[plotnum]]
      } # species loop
    } # mortality loop
    ...
    # add dummy column for code to work
    #if(dim(lfmorts)[2])
...
cols <- brewer.pal(nspecies,"Paired")

lfmorts.diff <- list()
lfmorts.diff[[1]] <- lfmorts.diff21
lfmorts.diff[[2]] <- lfmorts.diff31
...
for (plotnum in 1:length(fmortcols)){
    ...
    tiff(paste0(scenario.results.folder,'Scenario_differences_Lifetime_survival_F',
              fmortplot[plotnum]*100,'_',xlim,'km.tiff'), units="cm", 
              width=14, height=16, res=res)
  
    par(mfrow = c(3,2))
  
    for (scenario in 1:length(returndat)){ # loop over scenarios
      
      #age <- max.age #para.scenarios$age[scenario]
      #mean.dist <- para.scenarios$mean.dist[scenario]
      
      if(age == 'Max'){s.ylabel <- 'Survival (% individuals)'}
      ...
      #xlims <- c(10,100)
        ...
        #for(xl in 1:length(xlims)){
          ...
          #xlim <- xlims[xl]
            ...
            plot(MPAsizes/1000,lfmorts[scenario,,plotnum] * 100,
                 type='l',col=cols[scenario],lty=ltys[1],lwd=lwd,xlim = c(0,xlim),ylim = c(0,ylim.left),
            ...
            if (scenario>1){
              ...
              #if(length(dim(slfmorts.diff))==2){slfmorts.diff.dummy<-array(dim=c(dim(slfmorts.diff)[1],dim(slfmorts.diff)[2],2))
              #                                  slfmorts.diff.dummy[,,1]<-slfmorts.diff
              #                                  slfmorts.diff.dummy[,,2]<-slfmorts.diff
              #                                  slfmorts.diff <- slfmorts.diff.dummy}
              xlim.loc <- max(which(MPAsizes/1000<=xlim))
              ylim.min <- floor(min(slfmorts.diff[1:xlim.loc],na.rm=TRUE)*10)*10
              ylim.max <- ceiling(max(slfmorts.diff[1:xlim.loc],na.rm=TRUE)*10)*10
              
              plot(MPAsizes/1000,slfmorts.diff[,plotnum] * 100,
                   type='l',col=cols[scenario],lty=ltys,lwd=lwd,
          ...
          #    if(length(returndat)>1){
          #      for (s in 1:nspecies) {
          #        lines(MPAsizes/1000, slfmorts.diff[,plotnum] * 100,col=cols[s],lty=ltys[s],lwd=lwd)
          #      }
          #    }
